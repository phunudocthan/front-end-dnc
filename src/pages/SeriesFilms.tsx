...
const SeriesFilms = () => {
  ...
  const [phimBoTheLoaiId, setPhimBoTheLoaiId] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchTheLoais = async () => {
      try {
        ...
        const phimBo = (response.data.categories || []).find((tl: ITheLoai) => tl.tenTheLoai.toLowerCase().includes('bộ'));
        setPhimBoTheLoaiId(phimBo ? phimBo._id : null);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchFilms = async () => {
      try {
        ...
        console.log('SeriesFilms.tsx - res:', res);
        console.log('SeriesFilms.tsx - res.films:', res.films);
        setFilms(res.films);
        setTotalPages(res.totalPages || 1);
      } catch (error) {
        console.error('Error fetching series films:', error);
        setFilms([]);
        setTotalPages(1);
      }
    };
  ...
  return (
    <Container maxWidth="xl">
      <Box sx={{ mb: 4 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 6, p: 2, borderRadius: '8px', bgcolor: 'rgba(255,255,255,0.05)' }}>
          <Pagination
            count={totalPages}
            page={page}
