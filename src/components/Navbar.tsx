...
const yearList = [2024,2023,2022,2021,2020,2019,2018,2017,2016,2015,2014,2013,2012,2011,2010,'Trước 2009'];
...
const Navbar = () => {
  ...
  const handleSearchSubmit = (e: React.FormEvent<HTMLFormElement> | React.MouseEvent) => {
    e.preventDefault();
    if (searchValue.trim()) {
  ...
  return (
    ...
    >
      <Container maxWidth={false} sx={{ width: '100vw', overflow: 'visible', p: 0 }}>
        <Toolbar 
          disableGutters 
          sx={{ 
            px: { xs: 2, sm: 4 },
        ...
        >
            ...
            {menuItems.map((item, idx) => {
              ...
              if (item.type === 'dropdown') {
                ...
                return (
                  <Button
                    key={item.title}
                    endIcon={<ArrowDropDownIcon sx={{ fontSize: '1rem', ml: '2px', mr: '-6px' }} />}
                    sx={buttonSx}
                    onClick={onClick}
                  >
                    {item.title}
          ...
          {/* SearchBox */}
          <Box sx={{ display: 'flex', alignItems: 'center', ml: 'auto', flexShrink: 0, width: 'auto', justifyContent: 'flex-end' }}>
            <SearchBox onSubmit={handleSearchSubmit} sx={{ width: '140px', marginRight: 1, flexShrink: 0 }}>
              <StyledInputBase
                placeholder="Tìm kiếm..."
                value={searchValue}
                onChange={e => setSearchValue(e.target.value)}
                inputProps={{ 'aria-label': 'search', style: { height: 40, padding: 0, display: 'flex', alignItems: 'center' } }}
              />
              <IconButton
                type="submit"
                aria-label="search"
                sx={{ color: 'white', ml: 1, height: 40, width: 40, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            ...
            {/* Right Menu */}
            <Box sx={{ display: 'flex', alignItems: 'center', flexShrink: 0, ml: 1, mr: 2 }}>
              {isAuthenticated ? (
                <>
        ...
        <Menu
          ...
          sx={{ mt: 1, '& .MuiPaper-root': { backgroundColor: 'rgba(24, 24, 24, 0.95)', backdropFilter: 'blur(10px)', minWidth: 400, maxWidth: 700, p: 2, display: 'flex', flexWrap: 'wrap', gap: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.1)' } }}
        >
              ...
              <Box key={colIdx} sx={{ minWidth: 180, mr: 2 }}>
                {(theLoais || []).slice(
                  ...
                  <MenuItem
                    ...
                    onClick={() => {}}
                    sx={{ color: 'white', py: 1, '&:hover': { color: '#E50914', background: 'rgba(255,255,255,0.05)', borderRadius: '4px' } }}
                  >
                    {theLoai.tenTheLoai}
                  </MenuItem>
                ))}
        ...
        <Menu
          ...
          transitionDuration={0}
          sx={{ mt: 1, '& .MuiPaper-root': { backgroundColor: 'rgba(24, 24, 24, 0.95)', backdropFilter: 'blur(10px)', minWidth: 300, p: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.1)' } }}
        >
        ...
        <Menu
          ...
          transitionDuration={0}
          sx={{ mt: 1, '& .MuiPaper-root': { backgroundColor: 'rgba(24, 24, 24, 0.95)', backdropFilter: 'blur(10px)', minWidth: 220, p: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.1)' } }}
        >
          <Box sx={{ display: 'flex', flexWrap: 'nowrap', width: 220, alignItems: 'flex-start' }}>
            <Box sx={{ minWidth: 100, mr: 2, display: 'flex', flexDirection: 'column', gap: 0 }}>
              {(yearList || []).slice(0, 8).map((year) => (
                <MenuItem
                  key={year}
                  component={RouterLink}
                  to={`/namphathanh/${year === 'Trước 2009' ? 'before2009' : year}`}
                  onClick={() => {}}
                  sx={{ color: 'white', py: 1, minWidth: 100, '&:hover': { color: '#E50914', background: 'rgba(255,255,255,0.05)', borderRadius: '4px' } }}
                >
                  {year}
                </MenuItem>
              ))}
            </Box>
            <Box sx={{ minWidth: 100, display: 'flex', flexDirection: 'column', gap: 0 }}>
